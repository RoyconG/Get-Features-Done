---
name: gfd:convert-from-gsd
description: Migrate a GSD .planning/ project to GFD docs/features/ structure
argument-hint: (no arguments)
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, AskUserQuestion
---

<objective>Migrate a GSD .planning/ directory to GFD docs/features/ structure by scanning phases, presenting mappings for user review, and executing the migration.</objective>

<execution_context>
@/home/conroy/.claude/get-features-done/workflows/convert-from-gsd.md
@/home/conroy/.claude/get-features-done/references/ui-brand.md
</execution_context>

<process>Execute the convert-from-gsd workflow.</process>
